...
1. Установил консольное подключение к коммутатору с компьютера PC-A с помощью приложения Terminal.
...
1. ### **Проверка настройки коммутатора по умолчанию.**
...
1. **running configuration(коммутатор 2960)**:
...
3. **startup configuration в ОЗУ (NVRAM)**.
```
